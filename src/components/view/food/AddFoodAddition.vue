<template>
    ...
    ref="addFoodAdditionRef"
    v-model="isShowModal"
    :title="'Thêm sở thích phục vụ'"
    :isDrag="true"
    @btnCloseModalOnClick="btnCloseModalOnClick">
        <template #modal_content>
            <div class="modal_content">
                <BaseTextarea ref="inputFoodAdditionDescriptionRef" style="padding-bottom: 5px;" v-model="foodAddition.FoodAdditionDescription" :label="'Sở thích phục vụ'" :require="true"></BaseTextarea>
                <BaseInputInt @handleBlur="handleBlurFoodAdditionPrice" ref="inputFoodAdditionPriceRef" style="padding-bottom: 5px;" v-model="foodAddition.FoodAdditionPrice" :isMoney="true" :label="'Thu thêm'"></BaseInputInt>
            </div>
        </template>
...
export default {
    ...
    emits: ['closeDialog','insertFoodAddition'],
    data() {
        return {
            // Biến giữ cho modal luôn hiển thị
            isShowModal: true,
            // Biến lưu thông tin sở thích phục vụ
            foodAddition: {},
            // Biến kiểm tra thông tin form sở thích phục vụ có thay đổi không
            isFoodAdditionInfoChange: false,
    ...
    watch: {
        foodAddition: {
            handler: function(value){
                this.isFoodAdditionInfoChange = true;
            },
            deep: true
        },
    },
    methods: {
        ...
        /**
         * Hàm xử lý khi blur khỏi input nhập thu thêm
         * Author: Nguyễn Hoàng Anh (27/8/2022)
         */
        handleBlurFoodAdditionPrice() {
            if (!this.foodAddition.FoodAdditionPrice || this.foodAddition.FoodAdditionPrice < 0) {
                this.foodAddition.FoodAdditionPrice = 0;
            }
        },
        /**
            ...
            try {
                ...
                if (this.inputNotValid){
                    ...
                    this.$nextTick(() => {
                        var me = this
                        setTimeout(function() {
                            me.$refs.inputFoodAdditionDescriptionRef.$refs.inputRef.focus()
                        },50)
        ...
        validateBeforeSubmit() {
            try {
                var isValid = true;
                var FoodAdditionDescription = this.$refs.inputFoodAdditionDescriptionRef;
                var listMessageError = [];
                var listInputError = [];

                if (this.foodAddition.FoodAdditionDescription == undefined || !this.foodAddition.FoodAdditionDescription.trim()) {
                    FoodAdditionDescription.isValid = false;
                    listMessageError.push('Sở thích phục vụ không được để trống');
                    listInputError.push(FoodAdditionDescription.$refs.inputRef)
                    isValid = false;
                }
        ...
        btnSaveOnClick() {
            try {
                ...
                // 2. Build dữ liệu 
                this.foodAddition.FoodAdditionPrice = this.formatMoneyToDouble(this.foodAddition.FoodAdditionPrice)

                // 3. Gọi API
                FoodAdditionApi.post(this.foodAddition).then((res) => {
                    this.$emit('insertFoodAddition', this.foodAddition);
        ...
        btnCloseModalOnClick() {
            try {
                ...
                if (this.isFoodAdditionInfoChange == true){
    ...
    created() {
        this.foodAddition = {FoodAdditionDescription: '', FoodAdditionPrice: 0};

        var me = this
        setTimeout(function() {
            me.isFoodAdditionInfoChange = false;
            me.$refs.inputFoodAdditionDescriptionRef.$refs.inputRef.focus()
        }, 270);
    },
}
