    ...
    @btnCloseModalOnClick="btnCloseModalOnClick">
        <template #modal_content>
            <div class="modal_content">
                <BaseInput ref="inputFoodCategoryCodeRef" style="padding-bottom: 5px;" v-model="foodCategory.FoodCategoryCode" :label="'Mã nhóm'" :require="true"></BaseInput>
                <BaseInput ref="inputFoodCategoryNameRef" style="padding-bottom: 5px;" v-model="foodCategory.FoodCategoryName" :label="'Tên nhóm'" :require="true"></BaseInput>
                <BaseTextarea ref="inputFoodCategoryDescriptionRef" style="padding-bottom: 5px;" v-model="foodCategory.FoodCategoryDescription" :label="'Diễn giải'"></BaseTextarea>
            </div>
        </template>
...
export default {
    ...
    emits: ['closeDialog','insertFoodCategory'],
    data() {
        return {
            // Biến giữ cho modal luôn hiển thị
            isShowModal: true,
            // Biến lưu thông tin nhóm thực đơn
            foodCategory: {},
            // Biến kiểm tra thông tin form nhóm thực đơn có thay đổi không
            isFoodCategoryInfoChange: false,
    ...
    watch: {
        foodCategory: {
            handler: function(value){
                this.isFoodCategoryInfoChange = true;
            },
            deep: true
        },
    },
    methods: {
            ...
            try {
                ...
                if (this.inputNotValid){
                    ...
                    this.$nextTick(() => {
                        var me = this
                        setTimeout(function() {
                            me.$refs.inputFoodCategoryCodeRef.$refs.inputRef.focus()
                        },50)
        ...
        validateBeforeSubmit() {
            try {
                var isValid = true;
                var FoodCategoryCode = this.$refs.inputFoodCategoryCodeRef; 
                var FoodCategoryName = this.$refs.inputFoodCategoryNameRef;
                var listMessageError = [];
                var listInputError = [];

                if (this.foodCategory.FoodCategoryCode == undefined || !this.foodCategory.FoodCategoryCode.trim()) {
                    FoodCategoryCode.isValid = false;
                    listMessageError.push('Mã nhóm thực đơn không được để trống');
                    listInputError.push(FoodCategoryCode.$refs.inputRef)
                    isValid = false;
                }
                if (this.foodCategory.FoodCategoryName == undefined || !this.foodCategory.FoodCategoryName.trim()) {
                    FoodCategoryName.isValid = false;
                    listMessageError.push('Tên nhóm thực đơn không được để trống');
                    listInputError.push(FoodCategoryName.$refs.inputRef)
                    isValid = false;
                }
        ...
        btnSaveOnClick() {
            try {
                ...
                // 3. Gọi API
                FoodCategoryApi.post(this.foodCategory).then((res) => {
                    this.foodCategory.FoodCategoryID = res.data;
                    this.$emit('insertFoodCategory', this.foodCategory);
        ...
        btnCloseModalOnClick() {
            try {
                ...
                if (this.isFoodCategoryInfoChange == true){
    ...
    created() {
        ...
        var me = this
        setTimeout(function() {
            me.isFoodCategoryInfoChange = false;
            me.$refs.inputFoodCategoryCodeRef.$refs.inputRef.focus()
        }, 270);
